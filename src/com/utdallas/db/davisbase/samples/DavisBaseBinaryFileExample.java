...
public class DavisBaseBinaryFileExample {
	...
	public static void main(String[] args) {
		/*
		 * This method will initialize the database storage If there are any
		 */
		initializeDataStore();
		/*
		 * Create some user table. An actual NEW user table would begin with a
		 * single page. However, this example demonstrates how to add/remove
		 * increments of pages to/from files.
		 *
		 * Whenever the length of a RandomAccessFile is increased, the added
		 * space is padded with 0x00 value bytes.
		 */
		...
		try {
			...
			/*
			 * Re‐locate the address pointer at the beginning of page 2 and
			 * write 0x0D (b‐tree leaf node) to the first header byte
			 */
			binaryFile.seek(pageSize * 1);
			binaryFile.writeByte(0x0D);
			/*
			 * Re‐locate the address pointer at the beginning of page 3 and
			 * write 0x0D (b‐tree leaf node) to the first header byte
			 */
			 ...
			 * regardless of how long it currently is. The new bytes will be
			 * appended to the end and be all zeros
			 */
	 ...
	 *
	 * WARNING! Calling this method will destroy the system database catalog
